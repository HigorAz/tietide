...
- `NodeInput.scope?` — the full upstream output scope for the execution, keyed by
  source node id (`{ [nodeId]: thatNode'sOutputData }`). It's the same scope that
  already powers `{{nodeId.field}}` template resolution: `data` remains the flattened
  last-predecessor output, while `scope` exposes every already-executed node's output
  so executors can read sibling/ancestor results directly. The Code node surfaces it
