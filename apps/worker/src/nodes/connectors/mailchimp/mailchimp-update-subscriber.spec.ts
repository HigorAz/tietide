...
import { MailchimpUpdateSubscriberAction } from './mailchimp-update-subscriber';
import {
  MailchimpHttpError,
...
describe('MailchimpUpdateSubscriberAction', () => {
  let call: jest.Mock;
  let action: MailchimpUpdateSubscriberAction;

  beforeEach(() => {
    call = jest.fn();
    action = new MailchimpUpdateSubscriberAction(makeClient(call));
  });

  it('declares correct type and connection type', () => {
    expect(action.type).toBe('mailchimp-update-subscriber');
    expect(action.requiredConnectionType).toBe('mailchimp');
  });

  describe('happy path', () => {
    it('PATCHes the member with status and merge fields', async () => {
      call.mockResolvedValue({
        status: 200,
        data: { id: 'h', email_address: 'jane@example.com', status: 'subscribed' },
      } as MailchimpResponse);

      const ctx = makeContext({ getConnection: jest.fn().mockResolvedValue(makeConnection()) });
      const result = await action.execute(makeInput({ mergeFields: { FNAME: 'Jane' } }), ctx);

      const [, path, init] = call.mock.calls[0];
      expect(path).toMatch(/^\/lists\/abc123\/members\/[a-f0-9]{32}$/);
      expect(init.method).toBe('PATCH');
      const payload = JSON.parse(init.body as string);
      expect(payload).toMatchObject({ status: 'subscribed', merge_fields: { FNAME: 'Jane' } });
  ...
  describe('schema rejection', () => {
    it('rejects when neither status nor mergeFields is provided', async () => {
      const ctx = makeContext({ getConnection: jest.fn().mockResolvedValue(makeConnection()) });
      await expect(
        action.execute(
          {
