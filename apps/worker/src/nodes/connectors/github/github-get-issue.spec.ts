...
import { GitHubGetIssueAction } from './github-get-issue';
import {
  GitHubHttpError,
...
describe('GitHubGetIssueAction', () => {
  let call: jest.Mock;
  let client: ReturnType<typeof makeClient>;
  let action: GitHubGetIssueAction;

  beforeEach(() => {
    call = jest.fn();
    client = makeClient(call);
    action = new GitHubGetIssueAction(client as unknown as GitHubClientFactory);
  });

  it('declares correct type and required connection type', () => {
    expect(action.type).toBe('github-get-issue');
    expect(action.requiredConnectionType).toBe('github');
  });

  describe('happy path', () => {
    it('GETs /repos/:owner/:repo/issues/:n and maps the issue', async () => {
      ...
      expect(result.data).toMatchObject({ number: 7, title: 'Bug', state: 'open' });
    });
  });
