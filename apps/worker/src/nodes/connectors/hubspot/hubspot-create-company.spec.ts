...
import { HubspotCreateCompanyAction } from './hubspot-create-company';
import {
  HubspotHttpError,
...
const makeInput = (overrides: Partial<NodeInput['params']> = {}): NodeInput => ({
  ...
  params: { connectionId: VALID_CONNECTION_ID, name: 'Acme Inc', ...overrides },
});

describe('HubspotCreateCompanyAction', () => {
  let call: jest.Mock;
  let action: HubspotCreateCompanyAction;

  beforeEach(() => {
    call = jest.fn();
    action = new HubspotCreateCompanyAction(makeClient(call));
  });

  it('declares correct type and connection type', () => {
    expect(action.type).toBe('hubspot-create-company');
    expect(action.requiredConnectionType).toBe('hubspot');
  });

  describe('happy path', () => {
    it('POSTs name + domain properties to /crm/v3/objects/companies', async () => {
      call.mockResolvedValue({
        status: 201,
        data: { id: 'c-1', properties: { name: 'Acme Inc', domain: 'acme.com' } },
      } as HubspotResponse);

      const ctx = makeContext({ getConnection: jest.fn().mockResolvedValue(makeConnection()) });
      const result = await action.execute(makeInput({ domain: 'acme.com' }), ctx);

      const [, path, init] = call.mock.calls[0];
      expect(path).toBe('/crm/v3/objects/companies');
      expect(init.method).toBe('POST');
      const payload = JSON.parse(init.body as string);
      expect(payload.properties).toMatchObject({ name: 'Acme Inc', domain: 'acme.com' });
