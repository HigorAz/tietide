...
import { HubspotUpdateContactAction } from './hubspot-update-contact';
import {
  HubspotHttpError,
...
describe('HubspotUpdateContactAction', () => {
  let call: jest.Mock;
  let action: HubspotUpdateContactAction;

  beforeEach(() => {
    call = jest.fn();
    action = new HubspotUpdateContactAction(makeClient(call));
  });

  it('declares correct type and connection type', () => {
    expect(action.type).toBe('hubspot-update-contact');
    expect(action.requiredConnectionType).toBe('hubspot');
  });

  describe('happy path', () => {
    it('PATCHes /crm/v3/objects/contacts/:id with properties', async () => {
      call.mockResolvedValue({
        status: 200,
        data: { id: '501', properties: { lifecyclestage: 'customer' } },
      } as HubspotResponse);
      ...
      expect(payload.properties).toEqual({ lifecyclestage: 'customer' });
