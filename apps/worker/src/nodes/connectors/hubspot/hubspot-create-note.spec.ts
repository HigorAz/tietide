...
import { HubspotCreateNoteAction } from './hubspot-create-note';
import {
  HubspotHttpError,
...
const makeInput = (overrides: Partial<NodeInput['params']> = {}): NodeInput => ({
  ...
  params: { connectionId: VALID_CONNECTION_ID, body: 'Follow up next week', ...overrides },
});

describe('HubspotCreateNoteAction', () => {
  let call: jest.Mock;
  let action: HubspotCreateNoteAction;

  beforeEach(() => {
    call = jest.fn();
    action = new HubspotCreateNoteAction(makeClient(call));
  });

  it('declares correct type and connection type', () => {
    expect(action.type).toBe('hubspot-create-note');
    expect(action.requiredConnectionType).toBe('hubspot');
  });

  describe('happy path', () => {
    it('POSTs hs_note_body and associates a contact', async () => {
      call.mockResolvedValue({ status: 201, data: { id: 'note-1' } } as HubspotResponse);

      const ctx = makeContext({ getConnection: jest.fn().mockResolvedValue(makeConnection()) });
      const result = await action.execute(makeInput({ contactId: '501' }), ctx);

      const [, path, init] = call.mock.calls[0];
      expect(path).toBe('/crm/v3/objects/notes');
      expect(init.method).toBe('POST');
      const payload = JSON.parse(init.body as string);
      expect(payload.properties.hs_note_body).toBe('Follow up next week');
      expect(payload.properties.hs_timestamp).toBeDefined();
      expect(payload.associations[0].to.id).toBe('501');
      expect(payload.associations[0].types[0].associationTypeId).toBe(202);
      expect(result.data.id).toBe('note-1');
    });

    it('omits associations when no contact or deal is given', async () => {
      call.mockResolvedValue({ status: 201, data: { id: 'note-2' } } as HubspotResponse);
      const ctx = makeContext({ getConnection: jest.fn().mockResolvedValue(makeConnection()) });
      await action.execute(makeInput(), ctx);
      const payload = JSON.parse(call.mock.calls[0][2].body as string);
