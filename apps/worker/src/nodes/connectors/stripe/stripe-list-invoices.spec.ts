...
import { StripeListInvoicesAction } from './stripe-list-invoices';
import {
  StripeHttpError,
...
describe('StripeListInvoicesAction', () => {
  let call: jest.Mock;
  let action: StripeListInvoicesAction;

  beforeEach(() => {
    call = jest.fn();
    action = new StripeListInvoicesAction(makeClient(call));
  });

  it('declares correct type and connection type', () => {
    expect(action.type).toBe('stripe-list-invoices');
    expect(action.requiredConnectionType).toBe('stripe');
  });

  describe('happy path', () => {
    it('GETs /v1/invoices with customer + status filters', async () => {
      call.mockResolvedValue({
        status: 200,
        data: { data: [{ id: 'in_1' }, { id: 'in_2' }], has_more: false },
      } as StripeResponse);

      const ctx = makeContext({ getConnection: jest.fn().mockResolvedValue(makeConnection()) });
      const result = await action.execute(makeInput({ customerId: 'cus_1', status: 'paid' }), ctx);

      const [, path, init] = call.mock.calls[0];
      expect(path).toBe('/v1/invoices');
      expect(init.method).toBe('GET');
      expect(init.query).toMatchObject({ customer: 'cus_1', status: 'paid', limit: '10' });
