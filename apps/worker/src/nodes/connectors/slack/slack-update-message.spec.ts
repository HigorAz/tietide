...
import { SlackUpdateMessageAction } from './slack-update-message';
import {
  SlackHttpError,
...
function makeConnection(): DecryptedConnection<SlackOAuth2Config> {
  return {
    ...
    type: 'OAUTH2',
    provider: 'slack',
    config: { accessToken: 'xoxb-valid', teamId: 'T1', botUserId: 'U1', scope: 'chat:write' },
...
describe('SlackUpdateMessageAction', () => {
  let call: jest.Mock;
  let action: SlackUpdateMessageAction;

  beforeEach(() => {
    call = jest.fn();
    action = new SlackUpdateMessageAction(makeClient(call) as unknown as SlackClientFactory);
  });

  it('declares correct type and required connection type', () => {
    expect(action.type).toBe('slack-update-message');
    expect(action.requiredConnectionType).toBe('slack');
  });

  it('POSTs chat.update editing the message identified by ts', async () => {
    call.mockResolvedValue({
      status: 200,
      data: { ok: true, channel: 'C0123ABCDEF', ts: '1717000000.000100', text: 'edited text' },
    } as SlackResponse);

    const result = await action.execute(makeInput(), makeContext());

    const [, path, init] = call.mock.calls[0];
    expect(path).toBe('/chat.update');
    ...
    expect(result.data).toEqual({ ok: true, channel: 'C0123ABCDEF', ts: '1717000000.000100' });
  });
  ...
  it('rejects a malformed ts before any call', async () => {
    const ctx = makeContext();
    await expect(action.execute(makeInput({ ts: 'not-a-ts' }), ctx)).rejects.toThrow();
