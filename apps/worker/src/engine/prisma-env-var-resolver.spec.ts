...
describe('PrismaEnvVarResolver', () => {
  ...
  function setExecution(uid: string) {
  ...
  describe('getEnvScope', () => {
    it('should return GLOBAL vars when the user has no USER vars', async () => {
      setExecution(userId);
      prisma.environmentVariable.findMany.mockResolvedValue([
        { scope: 'GLOBAL', userId: null, key: 'BASE_URL', valueEnc: 'C1', valueNonce: 'N1' },
      ]);
    ...
    it('should let USER vars override GLOBAL vars on the same key', async () => {
      setExecution(userId);
      prisma.environmentVariable.findMany.mockResolvedValue([
        { scope: 'GLOBAL', userId: null, key: 'API_URL', valueEnc: 'GLOBAL_C', valueNonce: 'N' },
        { scope: 'USER', userId, key: 'API_URL', valueEnc: 'USER_C', valueNonce: 'N' },
      ]);
    ...
    it('should query Prisma with both scope filters and the resolved userId', async () => {
      setExecution(userId);
      prisma.environmentVariable.findMany.mockResolvedValue([]);
    ...
    it('should cache the result and not requery Prisma on the second call', async () => {
      setExecution(userId);
      prisma.environmentVariable.findMany.mockResolvedValue([
        { scope: 'GLOBAL', userId: null, key: 'K', valueEnc: 'C', valueNonce: 'N' },
      ]);
    ...
    it('should decrypt every row exactly once via CryptoService', async () => {
      setExecution(userId);
      prisma.environmentVariable.findMany.mockResolvedValue([
        { scope: 'GLOBAL', userId: null, key: 'A', valueEnc: 'CA', valueNonce: 'NA' },
        { scope: 'USER', userId, key: 'B', valueEnc: 'CB', valueNonce: 'NB' },
      ]);
  ...
  describe('isolation', () => {
    it('should never load USER rows belonging to a different user', async () => {
      // Prisma is mocked, so we assert the query — not the data filtering.
      // The query above tests the WHERE filter; here we just confirm cross-user
      // results would not be merged because the resolver uses scope+userId.
      setExecution(userId);
      prisma.environmentVariable.findMany.mockResolvedValue([
        { scope: 'USER', userId, key: 'OWN', valueEnc: 'C', valueNonce: 'N' },
      ]);

      const scope = await resolver.getEnvScope(executionId);

      expect(scope.has('OWN')).toBe(true);
      // Confirm WHERE includes scope=USER + userId — Prisma enforces the filter.
      ...
      expect(userClause.userId).toBe(userId);
    });
  });
});
