import { DriveFileUpdatedTrigger, DRIVE_FILE_UPDATED_TYPE } from './drive-file-updated.trigger';
...
describe('DriveFileUpdatedTrigger', () => {
  let trigger: DriveFileUpdatedTrigger;
  ...
  beforeEach(() => {
    ...
    trigger = new DriveFileUpdatedTrigger(factory);
  ...
  it('exports the canonical type string', () => {
    expect(DRIVE_FILE_UPDATED_TYPE).toBe('drive-file-updated');
    expect(trigger.type).toBe('drive-file-updated');
  });
  ...
  describe('onActivate', () => {
    it('watches the specific fileId and returns a composite providerSubId + 6.5d expiry', async () => {
    ...
    it('throws when config.fileId is missing', async () => {
      await expect(
        trigger.onActivate({
          workflowId: 'wf-1',
          nodeId: 'node-1',
    ...
    it('throws when the watch response is missing id or resourceId', async () => {
      drive.watch.mockResolvedValue({ data: { id: 'channel-uuid' } });
