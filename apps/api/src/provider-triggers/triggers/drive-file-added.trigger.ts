...
export const DRIVE_FILE_ADDED_TYPE = 'drive-file-added';
...
@Injectable()
export class DriveFileAddedTrigger extends BasePushTrigger {
  readonly type = DRIVE_FILE_ADDED_TYPE;
  readonly name = 'Drive: File Added';
  readonly description =
    'Triggers when Google Drive notifies that a watched folder changed (push, watch channel)';

  private readonly log = new Logger(DriveFileAddedTrigger.name);
  ...
  async onActivate(ctx: ActivationContext): Promise<ActivationResult> {
    ...
    const parentFolderId = cfg.parentFolderId;
    if (typeof parentFolderId !== 'string' || parentFolderId.length === 0) {
      throw new Error('drive-file-added requires config.parentFolderId');
    }
  ...
  async onDeactivate(ctx: DeactivationContext): Promise<void> {
    ...
    if (sep === -1) {
      throw new Error(
        `drive-file-added providerSubId is malformed (expected composite "<id>|<resourceId>")`,
      );
    }
