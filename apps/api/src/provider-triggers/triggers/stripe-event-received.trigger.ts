...
export const STRIPE_EVENT_RECEIVED_TYPE = 'stripe-event-received';
...
interface StripeNodeConfig {
  events?: string[];
}

@Injectable()
export class StripeEventReceivedTrigger extends BasePushTrigger {
  readonly type = STRIPE_EVENT_RECEIVED_TYPE;
  readonly name = 'Stripe: Event Received';
  readonly description = 'Triggers when a Stripe event is delivered to the workflow';

  private readonly log = new Logger(StripeEventReceivedTrigger.name);
  ...
  async onActivate(ctx: ActivationContext): Promise<ActivationResult> {
    ...
    const node = (ctx.config ?? {}) as StripeNodeConfig;
    const events = node.events && node.events.length > 0 ? node.events : ['*'];
